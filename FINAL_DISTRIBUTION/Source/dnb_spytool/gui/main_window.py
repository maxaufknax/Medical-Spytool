...
License: MIT License
    ...
    def show_publication_details(self, publication: Dict):
        """Show detailed information about a publication with enhanced schema support."""
        self.details_text.delete(1.0, tk.END)
        ...
        # Helper function to safely get and display fields
        ...
        try:
            # Build comprehensive details using enhanced schema
            details_sections = []
            
            # === BASIC INFORMATION ===
            basic_info = f"""=== BASIC INFORMATION ===
Title: {safe_get('title')}
Authors: {safe_join(publication.get('authors') or publication.get('author', []))}
Publication Year: {safe_get('publication_year')}
Publisher: {safe_get('publisher')}
Publication Type: {safe_get('publication_type') or safe_get('type')}
Database Source: {str(safe_get('database_source')).upper()}"""
            
            details_sections.append(basic_info)
            
            # === PUBLICATION METADATA ===
            metadata_info = f"""=== PUBLICATION METADATA ===
Journal: {safe_get('journal')}
Language: {safe_join(publication.get('language', []))}
Series: {safe_get('series')}
Physical Description: {safe_get('physical_description')}"""
            
            details_sections.append(metadata_info)
            
            # === IDENTIFIERS & LINKS ===
            # Handle URLs with enhanced logic
            ...
            # Generate automatic links for DOI, PubMed, PMC
            doi = safe_get('doi')
            pmid = safe_get('pmid')
            pmc = safe_get('pmc')
            ...
            identifiers_info = f"""=== IDENTIFIERS & LINKS ===
Record ID: {safe_get('id')}
...
Subject Headings: {safe_join(publication.get('subject_headings') or publication.get('subjects') or publication.get('subject', []))}
Notes: {safe_get('notes')}"""
            
            details_sections.append(content_info)
            
            # === METADATA SUMMARY ===
            # Count available enhanced fields
            ...
            enhanced_data_available = len(available_fields) > 4  # More than just basic fields
            
            summary_info = f"""=== METADATA SUMMARY ===
Enhanced Data Available: {'Yes' if enhanced_data_available else 'No'}
Available Data Fields: {len(publication.keys())}
Enhanced Fields Present: {len(available_fields)}/{len(enhanced_fields)}"""
            ...
            # Fallback to basic display with error information
            available_fields = list(publication.keys())
            details = f"""=== PUBLICATION DETAILS ===
Title: {safe_get('title')}
Authors: {safe_join(publication.get('authors') or publication.get('author', []))}
Publication Year: {safe_get('publication_year')}
Publisher: {safe_get('publisher')}
...
=== ERROR ===
Error displaying enhanced details: {str(e)}

=== DEBUG INFO ===
Available fields: {', '.join(available_fields)}
Total fields: {len(available_fields)}"""
