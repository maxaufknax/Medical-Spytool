...
class DNBSpytoolGUI:
    ...
    def create_results_tab(self):
        ...
        columns = ("Title", "Authors", "Year", "Publisher", "Type", "Abstract/Content", "Database")
        ...
        self.results_tree.heading("Authors", text="Autoren")
        self.results_tree.heading("Year", text="Jahr")
        self.results_tree.heading("Publisher", text="Verlag")
        self.results_tree.heading("Type", text="Typ")
        self.results_tree.heading("Abstract/Content", text="Abstract/Inhalt")
        self.results_tree.heading("Database", text="Datenbank")
        
        self.results_tree.column("Title", width=250)
        self.results_tree.column("Authors", width=160)
        self.results_tree.column("Year", width=60)
        self.results_tree.column("Publisher", width=150)
        self.results_tree.column("Type", width=100)
        self.results_tree.column("Abstract/Content", width=200)
        ...
        self.open_url_button = ttk.Button(url_frame, text="🌐 Open URL", 
...
License: MIT License
    ...
    def populate_results_tree(self):
        """Populate the results tree with publications including Abstract/Content column."""
        try:
            ...
            if not self.current_publications:
                self.results_count_label.config(text="Keine Ergebnisse")
                return
            ...
            for i, pub in enumerate(self.current_publications):
                ...
                title = str(pub.get('title', 'Kein Titel') or 'Kein Titel')[:100]
                ...
                # Extract abstract/content for new column
                abstract = pub.get('abstract', '') or pub.get('description', '') or pub.get('summary', '') or ''
                if abstract:
                    # Clean and truncate abstract
                    abstract_text = str(abstract).strip()[:150]
                    if len(str(abstract)) > 150:
                        abstract_text += "..."
                else:
                    abstract_text = ''
                
                database_source = str(pub.get('database_source', 'Unbekannt') or 'Unbekannt').upper()
                
                # Insert into tree with error handling including new abstract column
                try:
                    self.results_tree.insert('', 'end', values=(
                        title, authors, year, publisher, pub_type, abstract_text, database_source
                    ))
                except Exception as e:
                    print(f"Warnung: Publikation {i+1} konnte nicht eingefügt werden: {e}")
                    continue
                
                # Update progress for large datasets (every 50 items)
                if total_pubs > 100 and i % 50 == 0:
                    self.update_status(f"Lade Ergebnisse... {i+1}/{total_pubs}")
                    self.root.update_idletasks()
            
            # Enhanced results count with database breakdown
            database_counts = {}
            for pub in self.current_publications:
                db_source = str(pub.get('database_source', 'Unbekannt') or 'Unbekannt').upper()
                database_counts[db_source] = database_counts.get(db_source, 0) + 1
            
            count_text = f"{len(self.current_publications)} Publikationen"
            if len(database_counts) > 1:
            ...
            error_msg = f"Fehler beim Laden der Ergebnisse: {str(e)}"
            print(error_msg)
            self.results_count_label.config(text="Fehler beim Laden der Ergebnisse")
            self.update_status("Fehler beim Laden der Ergebnisse")
    ...
    def show_publication_details(self, publication: Dict):
        """Zeige detaillierte Informationen über eine Publikation mit deutschen Beschriftungen."""
        self.details_text.delete(1.0, tk.END)
        
        # Hilfsfunktion zum sicheren Verbinden von Werten
        ...
        # Hilfsfunktion zum sicheren Abrufen und Anzeigen von Feldern
        def safe_get(key, default=''):
            return publication.get(key, default) or default
        
        # Hilfsfunktion zum Kürzen langer Inhalte
        def truncate_content(content, max_length=500):
            if content and len(str(content)) > max_length:
                return str(content)[:max_length] + "..."
            return content
        
        try:
            # Umfassende Details mit deutschen Beschriftungen erstellen
            details_sections = []
            
            # === GRUNDINFORMATIONEN ===
            basic_info = f"""=== GRUNDINFORMATIONEN ===
Titel: {safe_get('title')}
Autoren: {safe_join(publication.get('authors') or publication.get('author', []))}
Erscheinungsjahr: {safe_get('publication_year')}
Verlag: {safe_get('publisher')}
Publikationstyp: {safe_get('publication_type') or safe_get('type')}
Datenbank-Quelle: {str(safe_get('database_source')).upper()}"""
            
            details_sections.append(basic_info)
            
            # === PUBLIKATIONS-METADATEN ===
            metadata_info = f"""=== PUBLIKATIONS-METADATEN ===
Zeitschrift: {safe_get('journal')}
Sprache: {safe_join(publication.get('language', []))}
Serie: {safe_get('series')}
Physische Beschreibung: {safe_get('physical_description')}"""
            
            details_sections.append(metadata_info)
            
            # === IDENTIFIKATOREN & LINKS ===
            # URLs mit verbesserter Logik verarbeiten
            primary_url = safe_get('url')
            all_urls = publication.get('all_urls', [])
            if isinstance(all_urls, str):
                all_urls = [all_urls]
            elif not isinstance(all_urls, list):
                all_urls = []
            
            # Automatische Links für DOI, PubMed, PMC generieren
            doi = safe_get('doi')
            pmid = safe_get('pmid')
            pmc = safe_get('pmc')
            ...
            if doi and doi.strip():
                auto_links.append(f"DOI: https://doi.org/{doi}")
            if pmid and pmid.strip():
                auto_links.append(f"PubMed: https://pubmed.ncbi.nlm.nih.gov/{pmid}/")
            if pmc and pmc.strip():
                auto_links.append(f"PMC: https://www.ncbi.nlm.nih.gov/pmc/articles/{pmc}/")
            
            identifiers_info = f"""=== IDENTIFIKATOREN & LINKS ===
Datensatz-ID: {safe_get('id')}
ISBN: {safe_join(publication.get('isbn', []))}
ISSN: {safe_get('issn')}
DOI: {doi}
PubMed ID: {pmid}
PMC ID: {pmc}

Primäre URL: {primary_url}
Zusätzliche URLs: {safe_join(all_urls) if all_urls else 'Keine'}"""
            
            if auto_links:
                identifiers_info += f"\nDirekte Links:\n" + "\n".join(auto_links)
            
            details_sections.append(identifiers_info)
            
            # === INHALT ===
            abstract = truncate_content(safe_get('abstract'))
            description = truncate_content(safe_get('description'))
            
            content_info = f"""=== INHALT ===
Abstract: {abstract or 'Nicht verfügbar'}
Beschreibung: {description or 'Nicht verfügbar'}
Sachgebiete: {safe_join(publication.get('subject_headings') or publication.get('subjects') or publication.get('subject', []))}
Anmerkungen: {safe_get('notes')}"""
            
            details_sections.append(content_info)
            
            # === METADATEN-ZUSAMMENFASSUNG ===
            # Verfügbare erweiterte Felder zählen
            enhanced_fields = ['id', 'abstract', 'description', 'journal', 'isbn', 'issn', 
                             'doi', 'pmid', 'pmc', 'url', 'all_urls', 'publisher', 'language', 
                             'subject_headings', 'publication_type', 'physical_description', 
                             'series', 'notes']
            
            available_fields = [field for field in enhanced_fields if publication.get(field)]
            enhanced_data_available = len(available_fields) > 4  # Mehr als nur Grundfelder
            
            summary_info = f"""=== METADATEN-ZUSAMMENFASSUNG ===
Erweiterte Daten verfügbar: {'Ja' if enhanced_data_available else 'Nein'}
Verfügbare Datenfelder: {len(publication.keys())}
Erweiterte Felder vorhanden: {len(available_fields)}/{len(enhanced_fields)}
Datenqualität: {'Hoch' if enhanced_data_available else 'Basis'}"""
            
            details_sections.append(summary_info)
            
            # Alle Abschnitte kombinieren
            details = "\n\n".join(details_sections)
            
        except Exception as e:
            # Fallback zu einfacher Anzeige mit Fehlerinformationen
            available_fields = list(publication.keys())
            details = f"""=== PUBLIKATIONSDETAILS ===
Titel: {safe_get('title')}
Autoren: {safe_join(publication.get('authors') or publication.get('author', []))}
Erscheinungsjahr: {safe_get('publication_year')}
Verlag: {safe_get('publisher')}
...
=== FEHLER ===
Fehler bei der Anzeige erweiterter Details: {str(e)}

=== DEBUG-INFORMATIONEN ===
Verfügbare Felder: {', '.join(available_fields)}
Anzahl Felder: {len(available_fields)}"""
        
        self.details_text.insert(1.0, details)
    
    def generate_analytics(self):
        """Generiere robuste Analytics für aktuelle Publikationen mit umfassender Fehlerbehandlung."""
        if not self.current_publications:
            messagebox.showwarning("Keine Daten", "Bitte führen Sie zuerst eine Suche durch, um Analytics zu generieren.")
            return
        
        try:
            self.update_status("Erstelle Analytics...")
            
            # Validierung und Bereinigung der Publikationsdaten vor der Analyse
            valid_publications = []
            total_original = len(self.current_publications)
            
            for i, pub in enumerate(self.current_publications):
                if pub is not None and isinstance(pub, dict):
                    # Weitere Validierung der Datenstruktur
                    if pub.get('title') or pub.get('authors') or pub.get('year'):
                        valid_publications.append(pub)
                    else:
                        print(f"Warnung: Publikation {i+1} hat unvollständige Grunddaten")
                else:
                    print(f"Warnung: Publikation {i+1} ist ungültig (None oder kein Dict)")
            
            if not valid_publications:
                messagebox.showwarning("Keine gültigen Daten", 
                                     f"Keine gültigen Publikationsdaten für Analytics gefunden.\n"
                                     f"Von {total_original} Einträgen waren 0 verwendbar.")
                return
            
            print(f"Analytics: {len(valid_publications)}/{total_original} Publikationen sind gültig")
            
            # Analyzer und Visualizer mit umfassender Fehlerbehandlung erstellen
            try:
                self.current_analyzer = PublicationAnalyzer(valid_publications)
                print("✅ PublicationAnalyzer erfolgreich erstellt")
                
                self.current_visualizer = PublicationVisualizer(self.current_analyzer)
                print("✅ PublicationVisualizer erfolgreich erstellt")
                
            except ImportError as e:
                messagebox.showerror("Modul-Fehler", 
                                   f"Analytics-Module konnten nicht geladen werden:\n{str(e)}\n\n"
                                   f"Bitte überprüfen Sie die Installation.")
                self.update_status("Analytics-Module nicht verfügbar")
                return
            except Exception as e:
                messagebox.showerror("Analytics-Fehler", 
                                   f"Analytics-Engine konnte nicht initialisiert werden:\n{str(e)}")
                self.update_status("Analytics-Initialisierung fehlgeschlagen")
                return
            
            # Zusammenfassende Statistiken mit umfassender Fehlerbehandlung generieren
            try:
                summary = self.current_analyzer.generate_summary_report()
                if summary and len(summary.strip()) > 50:
                    self.stats_text.delete(1.0, tk.END)
                    self.stats_text.insert(1.0, summary)
                    print("✅ Zusammenfassungsbericht erfolgreich generiert")
                else:
                    raise Exception("Leerer oder unvollständiger Bericht generiert")
                    
            except Exception as e:
                print(f"Warnung: Zusammenfassungserstellung fehlgeschlagen: {e}")
                # Fallback: Grundlegende Statistiken manuell erstellen
                try:
                    basic_stats = self._generate_fallback_statistics(valid_publications)
                    self.stats_text.delete(1.0, tk.END)
                    self.stats_text.insert(1.0, basic_stats)
                    print("✅ Fallback-Statistiken erfolgreich erstellt")
                except Exception as fallback_error:
                    # Letzter Fallback: Minimale Informationen
                    minimal_info = f"""=== MEDICAL SPYTOOL ANALYTICS ===

DATENÜBERSICHT:
- Gesamte Publikationen: {len(valid_publications)}
- Von ursprünglich: {total_original} Einträgen
- Gültigkeit: {len(valid_publications)/total_original*100:.1f}%

FEHLERMELDUNG:
Analytics konnte nicht vollständig generiert werden.
Grund: {str(e)}

Fallback-Fehler: {str(fallback_error)}

Die Suchergebnisse sind weiterhin verfügbar in der Ergebnisliste oben.
"""
                    self.stats_text.delete(1.0, tk.END)
                    self.stats_text.insert(1.0, minimal_info)
                    print("⚠️ Minimale Fallback-Informationen angezeigt")
            
            # Initiales Chart mit Fehlerbehandlung generieren
            try:
                self.update_chart()
                print("✅ Chart erfolgreich generiert")
            except Exception as e:
                print(f"Warnung: Chart-Erstellung fehlgeschlagen: {e}")
                # Weiter ohne Chart - Analytics-Text ist das Wichtigste
            
            self.update_status(f"Analytics erfolgreich erstellt für {len(valid_publications)} Publikationen")
            
        except Exception as e:
            error_msg = f"Unerwarteter Fehler bei Analytics-Erstellung: {str(e)}"
            print(f"❌ {error_msg}")
            messagebox.showerror("Analytics-Fehler", error_msg)
            self.update_status("Analytics-Erstellung fehlgeschlagen")
    
    def _generate_fallback_statistics(self, publications):
        """Generiere einfache Fallback-Statistiken wenn der Hauptanalyzer fehlschlägt."""
        try:
            ...
            for pub in publications:
                ...
                if publisher:
                    publishers.add(str(publisher).strip())
                
                # Datenbanken
                db = pub.get('database_source')
                if db:
                    databases.add(str(db).upper())
            
            # Statistiken zusammenstellen
            year_range = f"{min(years)} - {max(years)}" if years else "Unbekannt"
            
            fallback_stats = f"""=== MEDICAL SPYTOOL ANALYTICS (FALLBACK) ===

ÜBERSICHT:
- Gesamte Publikationen: {total_pubs}
- Eindeutige Autoren: {len(all_authors)}
- Jahresbereich: {year_range}
- Verschiedene Verlage: {len(publishers)}

DATENQUELLEN:
{chr(10).join([f"- {db}" for db in sorted(databases)])}

JAHR-VERTEILUNG:
{chr(10).join([f"- {year}: {years.count(year)} Publikationen" for year in sorted(set(years))[:5]])}

TOP AUTOREN:
{chr(10).join([f"- {author}" for author in sorted(all_authors)[:10]])}

HINWEIS:
Dies sind vereinfachte Statistiken aufgrund eines Analytics-Engine-Problems.
Die vollständigen Daten sind in der Ergebnisliste verfügbar.
"""
            return fallback_stats
            
        except Exception as e:
            return f"""=== ANALYTICS NICHT VERFÜGBAR ===

Es konnten keine Statistiken generiert werden.
Fehler: {str(e)}

Anzahl Publikationen: {len(publications)}

Bitte überprüfen Sie die Datenqualität und versuchen Sie es erneut.
"""
    ...
    def _get_publication_urls(self, publication: dict) -> list:
        """Get all available URLs for a publication."""
        urls = []
    ...
    def _select_url_dialog(self, urls: list) -> str:
