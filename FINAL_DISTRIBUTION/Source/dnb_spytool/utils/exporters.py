...
class DataExporter:
    ...
    def _export_csv(self, publications: List[Dict], output_path: str, 
        ...
        try:
            ...
            default_fieldnames = ['id', 'title', 'authors', 'publication_year', 'publisher', 
                               'isbn', 'subjects', 'abstract', 'description', 'languages', 'type', 'primary_url', 
            ...
            for pub in publications:
                ...
                flat_pub = {
                    ...
                    'abstract': pub.get('abstract', pub.get('description', pub.get('content', pub.get('summary', '')))),
    ...
    def _export_excel(self, publications: List[Dict], output_path: str, 
                     include_metadata: bool = True) -> bool:
        """Export to Excel format with guaranteed Abstract/Inhalt column."""
        try:
            # Ensure output directory exists
            output_dir = os.path.dirname(output_path)
            if output_dir:
            ...
            # Create the data with guaranteed Abstract/Inhalt column
            excel_data = []
            
            # Define fixed column order with Abstract/Inhalt guaranteed
            columns = [
                'ID', 'Title', 'Authors', 'Publication Year', 'Publisher', 
                'Abstract/Inhalt', 'Description', 'Subjects', 'Languages', 
                'Type', 'ISBN', 'Primary URL', 'DOI', 'PMID', 'PMC', 
                'All URLs', 'Database Source'
            ]
            
            for i, pub in enumerate(publications):
                # Extract abstract with multiple fallback options
                abstract_content = ''
                for field in ['abstract', 'description', 'content', 'summary', 'Abstract', 'ABSTRACT']:
                    if pub.get(field):
                        abstract_content = str(pub.get(field))
                        break
                
                # Handle authors
                authors_data = pub.get('authors', pub.get('author', []))
                ...
                # Handle subjects
                subjects_data = pub.get('subjects', pub.get('subject', []))
                if isinstance(subjects_data, list):
                    subjects_str = '; '.join(str(s) for s in subjects_data)
                else:
                    subjects_str = str(subjects_data) if subjects_data else ''
                
                # Handle ISBN
                isbn_data = pub.get('isbn', [])
                if isinstance(isbn_data, list):
                    isbn_str = '; '.join(str(i) for i in isbn_data)
                else:
                    isbn_str = str(isbn_data) if isbn_data else ''
                
                # Handle languages
                lang_data = pub.get('language', [])
                if isinstance(lang_data, list):
                    lang_str = '; '.join(str(l) for l in lang_data)
                else:
                    lang_str = str(lang_data) if lang_data else ''
                
                # Create row with GUARANTEED Abstract/Inhalt
                row_data = {
                    'ID': str(pub.get('id', '')),
                    'Title': str(pub.get('title', '')),
                    'Authors': authors_str,
                    'Publication Year': str(pub.get('publication_year', '')),
                    'Publisher': str(pub.get('publisher', '')),
                    'Abstract/Inhalt': abstract_content,  # GUARANTEED ABSTRACT COLUMN
                    'Description': str(pub.get('description', '')),
                    'Subjects': subjects_str,
                    'Languages': lang_str,
                    'Type': str(pub.get('type', '')),
                    'ISBN': isbn_str,
                    'Primary URL': str(pub.get('primary_url', '')),
                    'DOI': str(pub.get('doi', '')),
                    'PMID': str(pub.get('pmid', '')),
                    'PMC': str(pub.get('pmc', '')),
                    'All URLs': str(pub.get('all_urls', '')),
                    'Database Source': str(pub.get('database_source', 'Unknown'))
                }
            ...
            print(f"🔥 Abstract/Inhalt column exists: {'Abstract/Inhalt' in df.columns}")
            print(f"🔥 DataFrame shape: {df.shape}")
            
            if not df.empty:
                print(f"🔥 Sample Abstract/Inhalt values: {df['Abstract/Inhalt'].head(3).tolist()}")
            ...
            with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                ...
                if include_metadata:
                    metadata_data = [
                        ...
                        ['Abstract Column Guaranteed', 'YES'],
                        ['Format Version', '2.0-FIXED']
                    ]
