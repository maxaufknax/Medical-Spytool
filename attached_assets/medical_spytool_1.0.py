...
class MedicalSpyGUI:
    ...
    def save_current_query(self):
        ...
        query_name = tk.simpledialog.askstring("Suchanfrage speichern", "Name für die Suchanfrage:", initialvalue=display_name)
        if not query_name:
            return
    ...
    def display_chart(self):
        ...
        elif chart_type == "Autoren-Netzwerk":
            self.display_author_network()
    ...
    def display_author_network(self):
        """Zeigt ein einfaches Autoren-Netzwerk an"""
        try:
            import networkx as nx
        except ImportError:
            messagebox.showwarning("Modul fehlt", "Das Modul 'networkx' ist nicht installiert. "
                                 "Bitte installieren Sie es mit 'pip install networkx'.")
            return
            
        # Sammle Co-Autoren-Paare
        coauthor_pairs = []
        for result in self.search_results:
            authors_str = result.get("Autoren", "")
            if not authors_str:
                continue
        ...
        if not coauthor_pairs:
            messagebox.showinfo("Keine Daten", "Keine Co-Autoren-Informationen in den Ergebnissen gefunden.")
            return
        ...
        # Entferne isolierte Autoren
        G.remove_nodes_from(list(nx.isolates(G)))
        
        # Limitiere die Größe des Netzwerks für bessere Darstellung
        if len(G.nodes()) > 20:
            # Behalte nur die Autoren mit den meisten Verbindungen
            degrees = dict(G.degree())
            top_authors = sorted(degrees.items(), key=lambda x: x[1], reverse=True)[:20]
            top_author_names = [name for name, _ in top_authors]
            G = G.subgraph(top_author_names)
        ...
        # Kantendicke basierend auf Gewicht
        ...
        # Beschrifte nur die wichtigsten Knoten
        if len(G.nodes()) > 10:
        ...
        ax.set_title("Co-Autoren-Netzwerk")
        ax.axis('off')
        
        plt.tight_layout()
        
        self.display_figure(fig)
        log_message(self.text_log, "Autoren-Netzwerk aktualisiert.")
    ...
    def export_results(self):
        ...
        # Erstelle eindeutigen Dateinamen, wenn aktiviert
        if settings.get("unique_filenames", False):
            base, ext = os.path.splitext(out_path)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            out_path = f"{base}_{timestamp}{ext}"
            
        # Erstelle DataFrame aus Suchergebnissen
        df = pd.DataFrame(self.search_results)
        
        # Wähle ausgewählte Spalten
        selected_columns = [col for col, var in self.output_vars.items() if var.get()]
        ...
        # Exportiere basierend auf ausgewähltem Format
        try:
    ...
    def select_output_path(self):
        """Dialogfenster zum Auswählen des Ausgabepfads"""
        format_choice = self.combo_format.get()
        
        if format_choice == "Excel (.xlsx)":
            file_ext = ".xlsx"
            file_types = [("Excel Dateien", "*.xlsx")]
        elif format_choice == "CSV (.csv)":
            file_ext = ".csv"
            file_types = [("CSV Dateien", "*.csv")]
        elif format_choice == "JSON (.json)":
            file_ext = ".json"
            file_types = [("JSON Dateien", "*.json")]
        else:
            file_ext = ".xlsx"
            file_types = [("Excel Dateien", "*.xlsx")]
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            filetypes=file_types + [("Alle Dateien", "*.*")]
        )
        ...
        if file_path:
            ...
            # Aktualisiere Ausgabepfad in den Einstellungen
    ...
    def create_menu_bar(self):
        ...
        file_menu.add_command(label="Neue Suche", command=self.reset_search)
        file_menu.add_command(label="Öffnen...", command=self.open_results)
        file_menu.add_command(label="Speichern...", command=self.export_results)
        ...
        edit_menu.add_command(label="Kopieren", command=self.copy_selected_results)
        edit_menu.add_command(label="Alles auswählen", command=self.select_all_results)
        menubar.add_cascade(label="Bearbeiten", menu=edit_menu)
        
        # Ansicht-Menü
        view_menu = tk.Menu(menubar, tearoff=0)
        view_menu.add_command(label="Aktualisieren", command=self.refresh_view)
        menubar.add_cascade(label="Ansicht", menu=view_menu)
        
        # Extras-Menü
        tools_menu = tk.Menu(menubar, tearoff=0)
        tools_menu.add_command(label="Analyse", command=self.run_analysis)
        tools_menu.add_command(label="Diagramm anzeigen", command=self.display_chart)
        tools_menu.add_command(label="Einstellungen", command=lambda: self.notebook.select(self.tab_settings))
    ...
    def reset_search(self):
        ...
        log_message(self.text_log, "Suche zurückgesetzt.")
        
    def open_results(self):
        """Öffnet gespeicherte Ergebnisse aus einer Datei"""
        ...
        try:
            if file_path.endswith(".xlsx"):
                ...
                messagebox.showerror("Unbekanntes Format", "Das Dateiformat wird nicht unterstützt.")
                return
                
            # Konvertiere DataFrame zu Liste von Dictionaries
            self.search_results = df.to_dict('records')
            self.update_results_tree()
            
            messagebox.showinfo("Import erfolgreich", f"{len(self.search_results)} Ergebnisse aus {file_path} importiert.")
            log_message(self.text_log, f"Ergebnisse aus {file_path} importiert.")
            
        except Exception as e:
            messagebox.showerror("Import Fehler", f"Fehler beim Importieren der Ergebnisse: {e}")
            log_message(self.text_log, f"Fehler beim Importieren der Ergebnisse: {e}")
            
    def select_all_results(self):
        """Wählt alle Einträge in der Ergebnisliste aus"""
        for item in self.tree_results.get_children():
            self.tree_results.selection_add(item)
            
    def refresh_view(self):
        """Aktualisiert die Ansicht"""
        self.update_results_tree()
        log_message(self.text_log, "Ansicht aktualisiert.")
        
    def show_documentation(self):
        """Zeigt die Dokumentation an"""
        doc_text = """
        MedicalSpy - Wissenschaftliches Publikations-Suchtool
        
        Dieses Tool ermöglicht die Suche nach wissenschaftlichen und medizinischen Publikationen in verschiedenen 
        Datenbanken, darunter PubMed und die Deutsche Nationalbibliothek (DNB).
        
        Hauptfunktionen:
        
        1. Datenbankauswahl:
           - Wählen Sie die zu durchsuchende Datenbank im Tab "Datenbank" aus.
           
        2. Personenverwaltung:
           - Fügen Sie Personen mit Suchbegriffen im Tab "Personen & Suche" hinzu.
           - Sie können Personen importieren, exportieren, bearbeiten und löschen.
           
        3. Suchoptionen:
           - Stellen Sie erweiterte Suchparameter im Tab "Erweiterte Optionen" ein.
           - Definieren Sie Datumsbereich, Sprache und weitere Filter.
           
        4. Ausgabe:
           - Wählen Sie die Spalten für die Ausgabe im Tab "Ausgabe" aus.
           - Exportieren Sie Ergebnisse in verschiedenen Formaten.
           
        5. Analysefunktionen:
           - Analysieren Sie Ihre Suchergebnisse im Tab "Analyse".
           - Erstellen Sie verschiedene Visualisierungen.
           
        6. Gespeicherte Suchen:
           - Speichern und laden Sie Suchanfragen im Tab "Gespeicherte Suchen".
           
        Bei Fragen oder Problemen konsultieren Sie bitte die vollständige Dokumentation oder kontaktieren Sie den Entwickler.
        """
        
        dialog = tk.Toplevel(self.master)
        dialog.title("MedicalSpy - Dokumentation")
        ...
        btn_close = ttk.Button(dialog, text="Schließen", command=dialog.destroy)
    ...
    def show_about(self):
        ...
        messagebox.showinfo(
            ...
            "MedicalSpy - Wissenschaftliches Publikations-Suchtool\n"
            "Version 2.0\n\n"
            "Eine umfassende Anwendung zur Suche nach wissenschaftlichen\n"
            "und medizinischen Publikationen in verschiedenen Datenbanken.\n\n"
            "Erstellt auf Basis von DNB- und PubMed-Skripten."
        )
