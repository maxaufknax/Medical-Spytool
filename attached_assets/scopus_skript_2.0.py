...
# -*- coding: utf-8 -*-
"""
Scopus Publikations-Suchtool – Ultimate GUI
Erstellt von [Ihr Name]

Dieses Skript bietet eine GUI-Anwendung zur Suche in der Scopus-Datenbank.
Es umfasst Funktionen zur Personenverwaltung, erweiterte Suchoptionen, Ergebnisanzeige,
Logging, Analyse, Einstellungen sowie Exportfunktionen.
Die Suchfunktion ruft die Scopus-API (https://api.elsevier.com/content/search/scopus) auf,
parst die JSON-Antwort und extrahiert Felder wie Titel, Veröffentlichungsjahr, Autoren,
DOI, EID, Scopus URL und Zitationsanzahl.
Bitte ersetzen Sie SCOPUS_API_KEY durch Ihren echten API-Key.
"""
...
# Globaler API-Key für Scopus – bitte durch Ihren echten Schlüssel ersetzen!
...
# Globale Variablen
persons = []                # Liste der Personen (z. B. name, search_term, additional_search)
...
def load_settings():
    ...
    try:
        ...
        settings = {
            ...
            "output_columns": ["Name", "Titel", "Veröffentlichungsjahr", "Autoren", "DOI", "Scopus URL", "EID", "Zitationsanzahl"]
        }
...
def search_scopus(query, name, max_results=25):
    """
    Führt eine Suche in Scopus durch und gibt eine Liste von Publikations-Dictionaries zurück.
    """
    ...
    params = {
        ...
        "count": max_results,
        "start": 0,
        "view": "STANDARD"  # Sorgt dafür, dass alle benötigten Felder zurückgegeben werden.
    }
    try:
        ...
        log_message(None, f"Fehler bei der Scopus-Suche für Query '{query}': {e}")
        return []

##############################
# GUI-Klasse für Scopus
##############################
class ScopusGUI:
    def __init__(self, master):
        self.master = master
        self.master.title("Scopus Publikations-Suchtool - Ultimate GUI")
    ...
    def create_options_tab(self):
        ...
        ttk.Label(frame_opts, text="Sprache (optional):").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.entry_language = ttk.Entry(frame_opts, width=10)
        self.entry_language.grid(row=1, column=1, padx=5, pady=5)
    ...
    def create_output_tab(self):
        ...
        self.available_columns = settings.get("output_columns", ["Name", "Titel", "Veröffentlichungsjahr", "Autoren", "DOI", "Scopus URL", "EID", "Zitationsanzahl"])
        self.output_vars = {}
        columns_per_row = 2
    ...
    def create_results_tab(self):
        ...
        cols = settings.get("output_columns", ["Name", "Titel", "Veröffentlichungsjahr", "Autoren", "DOI", "Scopus URL", "EID", "Zitationsanzahl"])
    ...
    def create_settings_tab(self):
        ...
        ttk.Label(frame_settings, text="SCOPUS API Key:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.entry_api_key = ttk.Entry(frame_settings, width=40)
        self.entry_api_key.grid(row=0, column=1, padx=5, pady=5)
        self.entry_api_key.insert(0, SCOPUS_API_KEY)
    ...
    def run_search(self):
        ...
        for idx, person in enumerate(persons):
            ...
            final_query = f"({base_query}) AND ({additional})" if additional else base_query
            # Datumsauswahl: Filter auf Publikationsjahr
            start_date = self.entry_start_date.get().strip()
            end_date = self.entry_end_date.get().strip()
            if start_date and end_date:
                start_year = int(start_date.split("-")[0])
                end_year = int(end_date.split("-")[0])
                final_query += f" AND PUBYEAR AFT {start_year - 1} AND PUBYEAR BEF {end_year + 1}"
            # Optionale Sprache hinzufügen
            language = self.entry_language.get().strip()
            if language:
                final_query += f" AND LANGUAGE({language})"
            log_message(self.text_log, f"Suche für {person['name']} mit Query: {final_query}")
            results = search_scopus(final_query, person["name"], max_results=25)
    ...
    def show_about(self):
        messagebox.showinfo("Über", "Scopus Publikations-Suchtool\nVersion 1.0\nErstellt von [Ihr Name]")
