...
def check_python_version():
    ...
    if major >= 3 and minor >= 10:
        ...
        print_warning(
            "Bitte installieren Sie Python 3.10 oder höher von https://www.python.org/downloads/"
        )
        return False
...
def create_virtual_env():
    ...
    if venv_path.exists():
        choice = input(
            f"{Colors.WARNING}Virtuelle Umgebung existiert bereits. Neu erstellen? (j/n): {Colors.ENDC}"
        )
        if choice.lower() == "j":
            if platform.system() == "Windows":
...
def configure_environment():
    ...
    if env_file.exists():
        choice = input(
            f"{Colors.WARNING}.env-Datei existiert bereits. Überschreiben? (j/n): {Colors.ENDC}"
        )
        if choice.lower() != "j":
    ...
    if env_example.exists():
        ...
        env_content = env_content.replace(
            "SESSION_SECRET=your_secure_session_key_here", f"SESSION_SECRET={session_secret}"
        )
...
def check_database():
    ...
    if result and "Database connection successful" in result:
        ...
        print_warning(
            "Stellen Sie sicher, dass PostgreSQL läuft und die angegebene Datenbank existiert"
        )
        return False
...
def print_final_instructions():
    ...
    print(
        f"{Colors.GREEN}Die lokale Entwicklungsumgebung wurde erfolgreich eingerichtet!{Colors.ENDC}\n"
    )
