...
def main(args=None):
    ...
    try:
        ...
        if args.start_year or args.end_year:
            ...
            publications = apply_date_range_filter(publications, args.start_year, args.end_year)
            filtered_count = len(publications)
            if filtered_count < original_count:
                print(f"Date range filter applied: {original_count} → {filtered_count} publications")
...
Examples:
  ...
  %(prog)s --author "Einstein" --start-year 2000 --end-year 2020
  %(prog)s --gui
        """,
    ...
    parser.add_argument('--start-year', type=int,
                       help='Start year for date range filter (e.g., 2000)')
    parser.add_argument('--end-year', type=int,
                       help='End year for date range filter (e.g., 2024)')
...
def validate_cli_args(args, validator: InputValidator) -> bool:
    ...
    if args.start_year or args.end_year:
        date_validation = validate_date_range(args.start_year, args.end_year)
...
def validate_date_range(start_year: Optional[int], end_year: Optional[int]) -> Dict[str, Any]:
    """
    Validate date range parameters.
    ...
    if start_year is not None:
        if start_year < 1450:  # Earliest books
            errors.append("Start year must be 1450 or later")
        elif start_year > current_year:
            errors.append(f"Start year cannot be in the future (current year: {current_year})")
    
    if end_year is not None:
        if end_year < 1450:
            errors.append("End year must be 1450 or later")
        elif end_year > current_year + 5:  # Allow some future publications
            warnings.append(f"End year {end_year} is quite far in the future")
    
    # Validate range logic
    if start_year is not None and end_year is not None:
        if start_year > end_year:
            errors.append("Start year must be less than or equal to end year")
        elif end_year - start_year > 500:
            warnings.append("Very large date range selected - this may return many results")
...
def apply_date_range_filter(publications: List[Dict], start_year: Optional[int], end_year: Optional[int]) -> List[Dict]:
    """
    Filter publications based on publication year range.
    ...
    for pub in publications:
        pub_year = extract_publication_year(pub)
...
def extract_publication_year(publication: Dict) -> Optional[int]:
    """
